...
public final class PortExecutor {
    ...
    private final void exNotFoundClassPort(OftenRequest request, OftenResponse response,
            boolean responseWhenException) {
        response.toErr();
        if (responseWhenException) {
            JResponse jResponse = new JResponse(ResultCode.NOT_AVAILABLE);
    ...
    private ParamDealt.FailedReason paramDealOfPortInEntities(Context context,
            ...
            boolean isInClass, Porter porter, PorterOfFun porterOfFun, OftenObjectImpl oftenObjectImpl)
            throws Exception {
    ...
    private final void dealtOfResponse(OftenObjectImpl oftenObject, PorterOfFun porterOfFun, OutType outType,
            Object rs) {
        switch (outType) {
            case NO_RESPONSE:
                break;
    ...
    private void responseObject(OftenObjectImpl oftenObject, PorterOfFun porterOfFun, Object object,
            boolean nullClose) {
        if (object != null) {
            Logger LOGGER = logger(oftenObject);
            ...
            if (object instanceof JResponse && ((JResponse) object).isNotSuccess()) {
                ...
                if (throwable != null) {
                    ...
                    if (LOGGER.isWarnEnabled()) {
                        LOGGER.warn("{}:{}", oftenObject.url(), object);
