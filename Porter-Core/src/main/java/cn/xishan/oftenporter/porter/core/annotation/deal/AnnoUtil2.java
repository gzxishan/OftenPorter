 ...
 */
public class AnnoUtil2
{
    private static final Logger LOGGER = LoggerFactory.getLogger(AnnoUtil2.class);
    ...
    private static final String[] EXCEPT_CLASS_NAMES = {CacheKey.class.getName(), AnnoUtil.class.getName(),
            AnnoUtil.Advanced.class.getName()};
    private static Map<String, Integer> cacheCount;
    private static Map<String, List<IDynamicAnnotationImprovable>> classDynamicMap;
    private static List<IDynamicAnnotationImprovable> allDynamicList;
    private static List<IDynamicAnnotationImprovable> porterDynamicList;
    private static List<IDynamicAnnotationImprovable> aspectDynamicList;
    ...
    static
    {
        ...
        classDynamicMap = new HashMap<>();
        porterDynamicList = new ArrayList<>();
        aspectDynamicList = new ArrayList<>();
        allDynamicList = new ArrayList<>();
        try
        {
                ...
                for (String _classNames : dynamics)
                {
                    ...
                    for (String className : classNames)
                    {
                        IDynamicAnnotationImprovable iDynamicAnnotationImprovable = new DynamicAnnotationImprovableWrap(
                                className);
                        Set<String> supports = iDynamicAnnotationImprovable.supportClassNames();
                        for (String support : supports)
                        {
                            ...
                            if ("*".equals(support))
                            {
                                allDynamicList.add(iDynamicAnnotationImprovable);
                                continue;
                            }
                            List<IDynamicAnnotationImprovable> list = classDynamicMap.get(support);
                            if (list == null)
                            {
                                list = new ArrayList<>();
                                classDynamicMap.put(support, list);
                            }
                            list.add(iDynamicAnnotationImprovable);
                        }
                        if (iDynamicAnnotationImprovable.supportPorter())
                        {
                            porterDynamicList.add(iDynamicAnnotationImprovable);
                        }
                        if (iDynamicAnnotationImprovable.supportAspect())
                        {
                            aspectDynamicList.add(iDynamicAnnotationImprovable);
                        }
                    }
                }
            } catch (Exception e)
        ...
        DYNAMIC_ANNOTATION_IMPROVABLES_STRING = AnnoUtil2.class.getName() +
                "-classDynamics-" + classDynamicMap.hashCode();
    }
    ...
    {
        CacheKey cacheKey = new CacheKey(annotationClass, "getAnnotation[]");
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A) cache;
        }
        return cacheKey.setCache(NoCache.getAnnotation(annotations, annotationClass));
    }

    public static <A extends Annotation> A getAnnotation(Class<?> clazz, Class<A> annotationClass)
    {
        CacheKey cacheKey = new CacheKey(clazz, "getAnnotation", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A) cache;
        }
        return cacheKey.setCache(NoCache.getAnnotation(clazz, annotationClass));
    }

    public static <A extends Annotation> A getAnnotation(Field field, Class<A> annotationClass)
    {
        CacheKey cacheKey = new CacheKey(field, "getAnnotation", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A) cache;
        }
        return cacheKey.setCache(NoCache.getAnnotation(field, annotationClass));
    }

    public static <A extends Annotation> A getAnnotation(Method method, Class<A> annotationClass)
    {
        CacheKey cacheKey = new CacheKey(method, "getAnnotation", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A) cache;
        }
        return cacheKey.setCache(NoCache.getAnnotation(method, annotationClass));
    }

    public static <A extends Annotation> A getAnnotation(Parameter parameter, Class<A> annotationClass)
    {
        CacheKey cacheKey = new CacheKey(parameter, "getAnnotation", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A) cache;
        }
        return cacheKey.setCache(NoCache.getAnnotation(parameter, annotationClass));
    }
    ...
    public static Annotation[] getAnnotations(Class clazz)
    {
        ...
        return cacheKey.setCache(NoCache.getAnnotations(clazz));
    }

    public static Annotation[] getAnnotationsForAspectOperationOfPortIn(Porter porter)
    {
        CacheKey cacheKey = new CacheKey(porter, "aspect-portin", porter.getClazz());
        Object cache = cacheKey.getCache();
        if (cache != null)
        {
            if (cache == NULL)
    ...
    {
        CacheKey cacheKey = new CacheKey(clazz, "getAnnotationsWithSuper", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (List<A>) cache;
        ...
        return cacheKey.setCache(list);
    }

    public static <A extends Annotation> A[] getRepeatableAnnotations(Class<?> clazz, Class<A> annotationClass)
    {
        CacheKey cacheKey = new CacheKey(clazz, "getRepeatableAnnotations", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A[]) cache;
        }
        return cacheKey.setCache(NoCache.getRepeatableAnnotations(clazz, annotationClass));
    }

    public static <A extends Annotation> A[] getRepeatableAnnotations(Field field, Class<A> annotationClass)
    {
        CacheKey cacheKey = new CacheKey(field, "getRepeatableAnnotations", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A[]) cache;
        }
        return cacheKey.setCache(NoCache.getRepeatableAnnotations(field, annotationClass));
    }


    public static <A extends Annotation> A[] getRepeatableAnnotations(Method method, Class<A> annotationClass)
    {
        CacheKey cacheKey = new CacheKey(method, "getRepeatableAnnotations", annotationClass);
        Object cache = cacheKey.getCache();
        if (cache != null && cache != NULL)
        {
            return (A[]) cache;
        }
        return cacheKey.setCache(NoCache.getRepeatableAnnotations(method, annotationClass));
    }
    ...
    public static class Advance
    {
        ...
        private static <A extends Annotation> List<IDynamicAnnotationImprovable> getDynamic(Class<A> annotationType)
        {
            List<IDynamicAnnotationImprovable> list = classDynamicMap.get(annotationType.getName());
            if (list == null)
            {
        ...
        public static Annotation[] getAnnotationsForAspectOperationOfPortIn(Porter porter)
        {
            ...
            Annotation[] annotationResult = null;
            for (IDynamicAnnotationImprovable iDynamicAnnotationImprovable : porterDynamicList)
            {
        ...
        public static Annotation[] getAnnotationsForAspectOperationOfPortIn(PorterOfFun porterOfFun)
        {
            ...
            CacheKey cacheKey = new CacheKey(porterOfFun, "aspect-portfun", porterOfFun.getPorter().getClazz(),
                    porterOfFun.getMethod());
            Object cache = cacheKey.getCache();
            if (cache != null)
            {
                if (cache == NULL)
        ...
        public static AspectOperationOfNormal getAspectOperationOfNormal(Annotation annotation)
        {
            ...
            CacheKey cacheKey = new CacheKey(annotation, "AspectOperationOfNormal", annotation);
            Object cache = cacheKey.getCache();
            if (cache != null)
            {
                if (cache == NULL)
            ...
            {
                for (IDynamicAnnotationImprovable iDynamicAnnotationImprovable : aspectDynamicList)
                {
        ...
        public static AspectOperationOfPortIn getAspectOperationOfPortIn(Annotation annotation)
        {
            ...
            CacheKey cacheKey = new CacheKey(annotation, "AspectOperationOfPortIn", annotation);
